...
class ascending_base_agent(custom_agent):

    def __init__(self, args=None, agent_name='', obs_public_signal_dim=2, public_signal_range=10,
                 private_signal_range=10,
                 private_signal_generator=None):
        super(ascending_base_agent, self).__init__()
    ...
    def re_init_tmp_action(self):
        self.open_auction_record = []

    def get_last_tmp_action(self):
        if len(self.open_auction_record)>0:
            return self.open_auction_record[-1]
        else:
            return 0
    ...
    def record_tmp_action(self,action):
        self.open_auction_record.append(action)
    ...
    def receive_observation(self, args, budget, agent_name, agt_idx,
        ...
        #open asceding auction

        obs['highest_bid'] = extra_info['highest_bid']
        obs['tmp_allocation'] = extra_info['tmp_allocation']
    ...
    def generate_action(self, observation, public_signal_realization=None):
        ...
        # example agent generate the upbound_bid in the first round and gradually bid higher

        if len (self.open_auction_record) == 0 :
            # for the first version for temp eric:
            public_signal_realization = observation['public_signal']

            private_signal_realization = observation['private_signal']
            ...
            state = self.encode_signal(public_signal=public_signal_realization,
                                       private_signal=private_signal_realization)

            # here
            encoded_action = self.algorithm.generate_action(obs=state)

            # Decode the encoded action
            upbound_bid = encoded_action % (self.algorithm.bidding_range)

            # record
            self.record_action(upbound_bid)
            self.last_public_signal_realization = public_signal_realization
            self.last_encode_signal = state

        else:
            upbound_bid = self.get_latest_action()

        last_highest_bid = observation['highest_bid']
        minimal_raise = observation['minimal_raise']

        if last_highest_bid+minimal_raise <=upbound_bid :
            action = random.randint(last_highest_bid+minimal_raise, upbound_bid) # raise randomly
        else:
            action = upbound_bid
    ...
    def update_policy(self, obs, reward,done=False):
        ...
        # if receive the true value
        if 'true_value' in obs and 'payment' in obs:
            winner_payment = obs['payment']
            true_value = obs['true_value']

            # if obs['allocation'] ==0 :
            #     # not win --> reward =0 -> reward = true_value - provide bid (if win)
            #     reward = true_value - min(winner_payment,last_action) # second price or the payment
    ...
    def encode_signal(self, public_signal, private_signal=None):
        ...
        elif public_signal is None:
            # simple regard as private signal as example
            cnt = 0
            for obs_signal in private_signal:

                # check if it is none:
                if obs_signal is None:
                    print('exist None in ' + str(self.agent_name) + 'signal_dim is ' + str(obs_signal))
                else:
                    state += obs_signal * ((self.private_signal_range + 1) ** cnt)  # [0-H]
                cnt += 1

        else:
            print('not finish for the both signal exist !')
