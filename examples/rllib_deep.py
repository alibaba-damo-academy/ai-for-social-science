...
parser.add_argument('--player-num', type=int, default='4',
                    help='Set agent number')
parser.add_argument('--second-price', type=bool, default=False,
                    help='Set second price or not')
parser.add_argument('--estimate-frequency', type=int, default=100,
                    help='estimate_frequency')

parser.add_argument('--folder-name', type=str, default='results/test_yang',
                    help='store_figure path(should be made by yourself XD)')
parser.add_argument('--print-key', type=bool, default=False,
                    help='whether to print procedure')

parser.add_argument('--bidding-range', type=int, default=100,
                    help='whether to print procedure')
...
def env_creator(player_num=2,action_space_num=100,second_price=True):
    env = customed_env(player_num=player_num, action_space_num=action_space_num, second_price=second_price,env_iters =training_env_step)
    env.reset()
    return env
...
def test_policy(algo,test_num=10,player_num=2, action_space_num=100, second_price=True):
    test_env = customed_env(player_num=player_num, action_space_num=action_space_num,
                            second_price=second_price,env_iters =test_num)
    test_env.reset()
    import copy
...
def test_estimation(algo,plot=False,epoch=0,folder_name='deep',
    ...
    # Construct the environment for plotting
    ...
    if plot:
        ...
        #plt.yticks(y_ticks)
        plt.legend()
        Path('./' + folder_name + args.algorithm+'/').mkdir(parents=True, exist_ok=True)
        print('./' + folder_name + args.algorithm+'/')
        plt.savefig('./' + folder_name + args.algorithm+'/'+ same_valuation_name + overbid_fig_name + 'test_players' + str(
...
def run_same_policy(args, stop,algorithm="DQN",estimate_frequency=100):
    ...
    config = {
        ...
        "num_gpus": args.num_gpus, # int(os.environ.get("RLLIB_NUM_GPUS", "3")),
        "num_workers": 0,
...
if __name__ == "__main__":
    ...
    register_env("second_auction", lambda config: PettingZooEnv(env_creator(player_num=args.player_num,
                                                                            action_space_num=args.bidding_range,
                                                                            second_price=args.second_price)))
    run_same_policy(args, stop=stop, algorithm = args.algorithm , estimate_frequency=args.estimate_frequency)
