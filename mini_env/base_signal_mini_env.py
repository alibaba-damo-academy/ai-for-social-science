...
class signal_mini_env(dynamic_env):
    def __init__(self, round, item_num, mini_env_iters, agent_num, args, policy, set_env=None, init_from_args=True,
        ...
        super(signal_mini_env).__init__()
    ...
    def check_private_signal_generator(self, agent_name):
        if self.private_signal_generator is None or (
        not self.private_signal_generator.check_avaliable_agent_name(agent_name=agent_name)):
            return False
        else:
            return True
        ...
        for agent_name in env.agent_iter():
            ...
            if (self.mini_item == 1 and (
                ...
                if not self.winner_only or allocation:
                    ...
                    obs = increase_info_to_obs(obs, extra_info_name='final_pay', value=info['final_pay'])
    ...
    def print_results(self,env,args,new_action,agent_name,next_true_value,last_k_epoch=None):
        ...
        if use_old:
            ...
            if ((epoch / args.player_num) % args.revenue_averaged_stamp == 0) and (agent_name == agents_list[-1]) and ignore_explore_flag:
                #record all
