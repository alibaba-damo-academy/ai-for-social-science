...
# from test_mini_env.torch_ppo_example import *
...
parser.add_argument('--gpu', type=str, default='4,3',
...
# Xue added
# deep learning realted configuration
parser.add_argument('--mode', type=str, default='pytorch',help='please choose the model mode, such as pytorch, mode1, pytorch_v2 ')

parser.add_argument('--rl_lib_available', action='store_true', help='use multiple gpus', default=False)

parser.add_argument('--pytorch_lib_avalable', action='store_true', help='use multiple gpus', default=True)

parser.add_argument('--only_winner_know_true_value', action='store_true', help='use multiple gpus', default=False)

parser.add_argument('--method', default = 'ppo',help='dqn or ppo')
...
def easy_signal_setting(args,mode='default'):
    ...
    if mode =='default':
        ...
        args.exp_id=404  # 304 for cfr v1
        # 306 new exp for new cfr v2
        if args.exp_id==304 or args.exp_id==305 or args.exp_id==306:
            args.exploration_epoch = 50000  # 0
            args.step_floor = 20000
            args.env_iters = 1000000  # 0
            args.estimate_frequent = 100000  # 100000
            args.batchsize = 64

        if args.exp_id==300 or args.exp_id==302:
            ...
            args.batchsize=1024
            args.lr=5e-4
            args.exploration_epoch = 0

            args.env_iters = 200000*360  # 0
            args.step_floor = 100000
            args.estimate_frequent = 100000  # 100000

        if args.exp_id in [200,202,400,404]:
            # 202 | 200 is the v2 update policy
            args.cfr_loss = 0
            if args.exp_id == 202 or args.exp_id ==404:
                args.cfr_loss = 1
                print('open cfr')
...
# mode = 'pytorch' #'mode1' #'pytorch_v2'
# rl_lib_available=False
# pytorch_lib_avalable= True
...
def main(args):
    ...
    # easy mode change
    args=easy_signal_setting(args,mode=args.mode)



    if args.method == 'dqn':
        from test_mini_env.pytorch_algo_example import \
            get_custom_algorithm_in_pytorch,\
            generate_action_pytorch,\
            update_policy_pytorch,\
            receive_observation_pytorch,\
            test_policy_pytorch
    elif args.method == 'ppo':
        from test_mini_env.torch_ppo_example import \
            get_custom_algorithm_in_pytorch,\
            generate_action_pytorch,update_policy_pytorch_v2,update_policy_pytorch_v_diff,\
            update_policy_pytorch,\
            receive_observation_pytorch,\
            test_policy_pytorch
    ...
    dynamic_env = signal_mini_env(round=args.round, item_num=args.item_num, mini_env_iters=10000,
                                  ...
                                  signal_type='normal',winner_only=args.only_winner_know_true_value
        )
    ...
    while dynamic_env.get_current_round() < dynamic_env.get_round(): #only play once
        # print(dynamic_env.get_current_round(), dynamic_env.get_round())
        ...
        if args.pytorch_lib_avalable:
            ...
            def set_pytorch_for_agent(custom_agent_name,dynamic_env,custom_algorithm):
                ...
                dynamic_env.set_rl_agent_generate_action(generate_action=generate_action_pytorch,
                                                         agent_name=custom_agent_name)
                if args.exp_id == 202 or args.exp_id ==200:
                    print('use v2 update policy')
                    dynamic_env.set_rl_agent_update_policy(update_policy=update_policy_pytorch_v2,
                                                           agent_name=custom_agent_name)  # add function search agt.update_policy | user can user other infos in basic agents(see in  normal_agent.py )
                elif args.exp_id ==404 or args.exp_id == 400:
                    dynamic_env.set_rl_agent_update_policy(update_policy=update_policy_pytorch_v_diff,
                                                           agent_name=custom_agent_name)  # add function search agt.update_policy | user can user other infos in basic agents(see in  normal_agent.py )
            ...
            # SET FOR ALL AGENT
            set_pytorch_for_agent(custom_agent_name='player_0',dynamic_env=dynamic_env,custom_algorithm=get_custom_algorithm_in_pytorch(args,seed=0))
        ...
        adjust_dynamic_env(dynamic_env,args=args,mode=args.mode)


        #next step
        dynamic_env.step() #run the mini env and update policy each
        print(dynamic_env.step())
        # rebuild mini env
